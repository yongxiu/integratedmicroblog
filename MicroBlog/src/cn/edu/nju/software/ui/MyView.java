...
public class MyView extends LinearLayout {
	...
	private Handler myHandler = new HomeHandler();

	public MyView(Context activity, AttributeSet attrs) {
	...
	private void init(Activity activity) {
		...
		addView(li.inflate(R.layout.my, null));

		msgList = (ListView) findViewById(R.my.Msglist);
		refreshBtn = (ImageButton) findViewById(R.my.StatusRefresh);
					...
					new Thread() {
						public void run() {
							getMore(Long.parseLong(((StatusItem) arg0
									...
									.getId()));
							myHandler.sendEmptyMessage(REFRESH_COMPLETE);
						}
					}.start();
				} else {
					Serializable status = (Serializable) view.getTag();
					Intent intent = new Intent(MyView.this.activity,
	...
	private void getMore(long maxId) {
		StatusItem[] status = UserServiceImpl.getService().getUserTimeLine(
				activity, 0, maxId - 1);
